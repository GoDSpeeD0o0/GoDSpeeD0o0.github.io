(() => {
  ...
  function initMetricCounters() {
    ...
    metrics.forEach(metric => {
      ...
      const hasSign = targetValue.startsWith('+') || targetValue.startsWith('-');
      const sign = hasSign ? targetValue[0] : '';
      const numValue = parseInt(targetValue.replace(/[+-]/g, ''), 10);
        ...
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const animate = () => {
              current += increment;
              if (current < numValue) {
                metric.textContent = sign + Math.floor(current);
                requestAnimationFrame(animate);
              } else {
                metric.textContent = sign + numValue + (isNegative ? '%' : '');
              }
            };
            animate();
            io.unobserve(metric);
